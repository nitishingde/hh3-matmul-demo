...
#ifndef HH3_MATMUL_PARTIAL_COMPUTATION_STATE_H
#define HH3_MATMUL_PARTIAL_COMPUTATION_STATE_H

#include "../data/matrix_block_data.h"

template<class Type, Order Ord>
class PartialComputationState: public hh::State<2,
    ...
    > {
private:
    size_t gridHeightResults_ = 0;
    size_t gridWidthResults_ = 0;
    std::vector<std::vector<std::shared_ptr<MatrixBlockData<Type, 'p', Ord>>>> gridPartialProduct_ = {};
    std::vector<std::shared_ptr<MatrixBlockData<Type, 'c', Ord>>> gridMatrixC_ = {};
    size_t ttl_ = 0;

public:
    PartialComputationState(size_t gridHeightResults, size_t gridWidthResults, size_t ttl): gridHeightResults_(gridHeightResults), gridWidthResults_(gridWidthResults), ttl_(ttl) {
        gridPartialProduct_ = std::vector<std::vector<std::shared_ptr<MatrixBlockData<Type, 'p', Ord>>>>(gridHeightResults_ * gridWidthResults_);
        gridMatrixC_ = std::vector<std::shared_ptr<MatrixBlockData<Type, 'c', Ord>>>(gridHeightResults_ * gridWidthResults_, nullptr);
    }

    void execute(std::shared_ptr<MatrixBlockData<Type, 'c', Ord>> ptr) override {
        auto i = ptr->rowIdx(), j = ptr->colIdx();
        if(isPAvailable(i, j)) {
            auto res = std::make_shared<std::pair<std::shared_ptr<MatrixBlockData<Type, 'c', Ord>>, std::shared_ptr<MatrixBlockData<Type, 'p', Ord>>>>();
            res->first = ptr;
...
#endif //HH3_MATMUL_PARTIAL_COMPUTATION_STATE_H
