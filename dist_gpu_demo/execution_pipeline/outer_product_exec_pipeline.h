...
public:
    explicit OuterProductExecPipeline(
        std::shared_ptr<hh::Graph<2, MatrixTile<MatrixType, InpIdA, Ord>, MatrixTile<MatrixType, InpIdB, Ord>, MatrixTile<MatrixType, OutId, Ord>>> const &graph,
        std::vector<int> const &deviceIds
    ): hh::AbstractExecutionPipeline<2, MatrixTile<MatrixType, InpIdA, Ord>, MatrixTile<MatrixType, InpIdB, Ord>, MatrixTile<MatrixType, OutId, Ord>>(graph, deviceIds) {
        deviceCount_ = deviceIds.size();
    }

    bool sendToGraph(std::shared_ptr<MatrixTile<MatrixType, InpIdA, Ord>> &matrixTileA, size_t const &graphId) override {
        return (matrixTileA->colIdx() % deviceCount_) == graphId;
    }

    bool sendToGraph(std::shared_ptr<MatrixTile<MatrixType, InpIdB, Ord>> &matrixTileB, size_t const &graphId) override {
