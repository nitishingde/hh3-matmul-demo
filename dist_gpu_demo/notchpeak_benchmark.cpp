...
int main([[maybe_unused]]int32_t argc, [[maybe_unused]]char **argv) {
    ...
    auto strategy = MMD_MpiOuterProduct2<MatrixType, 'a', 'b', 'c', Ord>(productThreads, commThreads);
    ...
    for(uint32_t iter = 0; iter < ITER; ++iter) {
        ...
        strategy.executeImpl(subMatA, subMatB, matrixC, deviceIds, "MMD_Unified_iter"+std::to_string(iter)+"_node");
        MPI_Barrier(MPI_COMM_WORLD);
