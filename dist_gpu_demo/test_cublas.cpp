...
int main(int argc, char **argv) {
    ...
    uint32_t M = 8, K = 6, N = 8, tileSize = 2;
    ...
    if(isRootNodeId()) {
        ...
        for (uint32_t j = 0, width, nodeId = 0; j < K; ++nodeId) {
            width = K/getNumNodes();
            width += (nodeId < K%getNumNodes()? 1: 0);
            MatrixType val = nodeId+1;
            for(uint32_t k = 0; k < width; ++k, ++j) {
        ...
        for(uint32_t i = 0, height, nodeId = 0; i < K; ++nodeId, i+=height) {
...
#endif
        for(uint32_t i = 0; i < testC.size(); ++i) {
