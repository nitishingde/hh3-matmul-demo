...
template<class MatrixType, char IdA, char IdB, char IdC>
class MMD_WindowStrategy2: public MMD_Strategy<MatrixType, IdA, IdB, IdC> {
private:
    using MatrixA = MMD_Strategy<MatrixType, IdA, IdB, IdC>::MatrixA;
    using MatrixB = MMD_Strategy<MatrixType, IdA, IdB, IdC>::MatrixB;
    using MatrixC = MMD_Strategy<MatrixType, IdA, IdB, IdC>::MatrixC;
...
public:
    ...
    double executeImpl(
        ...
        auto jobGenTask         = std::make_shared<GpuJobGeneratorTask2<MatrixType, IdA, IdB, IdC>>(gp_, gq_, windowHeight_, windowWidth_, graphFilterState);
        jobGenTask->connectMemoryManager(std::make_shared<GpuTokenMemoryManager>(deviceIds));
        auto tileSorterTask     = std::make_shared<TileSorterTask<MatrixType, IdA, IdB>>(gp_, gq_);
        auto execPipeline       = std::make_shared<OuterProductExecutionPipeline<MatrixType, IdA, IdB, IdC>>(
            std::make_shared<OuterProductGpuGraph<MatrixType, IdA, IdB, IdC>>(MT, KT, NT, T, windowHeight_, windowWidth_, depth_, productThreads_),
        ...
        graph.template edge<MatrixB>(inputStateManager, dwTaskB);
        graph.template edge<Triplet>(inputStateManager, jobGenTask);
        graph.template edge<DwBatchRequest<IdA>>(jobGenTask, dwTaskA);
        graph.template edge<DwBatchRequest<IdB>>(jobGenTask, dwTaskB);
        graph.template edge<TileA>(dwTaskA, tileSorterTask);
        graph.template edge<TileB>(dwTaskB, tileSorterTask);
        graph.template edge<TileA>(tileSorterTask, execPipeline);
        graph.template edge<TileB>(tileSorterTask, execPipeline);
...
public:
    ...
    MMD_Strategy<MatrixType, IdA, IdB, IdC>& builder(const int64_t gp, const int64_t gq, const int64_t jobHeight, const int64_t jobWidth, const int64_t depth, const int64_t lookAhead, const int64_t productThreads) {
        gp_             = gp;
        gq_             = gq;
        jobHeight_      = jobHeight;
        jobWidth_       = jobWidth;
        depth_          = depth;
        lookAhead_      = lookAhead;
        productThreads_ = productThreads;
    ...
    double executeImpl(
        ...
        auto jobGenTask         = std::make_shared<GpuJobGeneratorTask3<MatrixType, IdA, IdB, IdC>>(gp_, gq_, jobHeight_, jobWidth_, graphFilterState);
        jobGenTask->connectMemoryManager(std::make_shared<GpuTokenMemoryManager>(deviceIds));
        auto tileSorterTask     = std::make_shared<TileSorterTask<MatrixType, IdA, IdB>>(gp_, gq_);
        auto execPipeline       = std::make_shared<OuterProductExecutionPipeline<MatrixType, IdA, IdB, IdC>>(
            std::make_shared<OuterProductGpuGraph<MatrixType, IdA, IdB, IdC>>(MT, KT, NT, T, jobHeight_, jobWidth_, depth_, productThreads_),
            deviceIds,
            graphFilterState
        );
        ...
        graph.template input<Triplet>(inputStateManager);
        graph.template edge<Triplet>(inputStateManager, jobGenTask);
        graph.template edge<CommRequestList<IdA>>(jobGenTask, commTaskA);
        graph.template edge<CommRequestList<IdB>>(jobGenTask, commTaskB);
        graph.template edge<CommSendList<IdA>>(jobGenTask, commTaskA);
        graph.template edge<CommSendList<IdB>>(jobGenTask, commTaskB);
        graph.template edge<TileA>(commTaskA, tileSorterTask);
        graph.template edge<TileB>(commTaskB, tileSorterTask);
        graph.template edge<TileA>(tileSorterTask, execPipeline);
        graph.template edge<TileB>(tileSorterTask, execPipeline);
        graph.template edge<Job>(jobGenTask, execPipeline);
        graph.template output<TileC>(execPipeline);
