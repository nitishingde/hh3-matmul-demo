...
class GpuJobGeneratorTask3: public hh::AbstractTask<
        ...
        MatrixTile<MatrixType, IdA>,
        MatrixTile<MatrixType, IdB>,
        BroadcastBatchRequest<IdA>,
        BroadcastBatchRequest<IdB>,
        GpuJob<MatrixType, IdA, IdB, IdC>
    > {
private:
    using MatrixA = MatrixContainer<MatrixType, IdA>;
...
public:
    explicit GpuJobGeneratorTask3(const size_t gp, const size_t gq, const int64_t windowHeight, const int64_t windowWidth, std::shared_ptr<GraphFilterState> &graphFilterState):
        hh::AbstractTask<1, Triplet, TileA, TileB, BroadcastBatchRequest<IdA>, BroadcastBatchRequest<IdB>, Job>("GpuJobGeneratorTask", 1, false),
        gp0_(gp), gq0_(gq), windowHeight_(windowHeight), windowWidth_(windowWidth), graphFilterState_(graphFilterState) {}
    ...
    void execute(std::shared_ptr<Triplet> triplet) override {
        ...
        for(size_t i = 0; i < rowIndices.size(); i+= (gp0_*windowHeight_)) {
            for(size_t j = 0; j < colIndices.size(); j+= (gq0_*windowWidth_)) {
                ...
                auto reqA = std::make_shared<BroadcastBatchRequest<IdA>>(batchJobCount == 1);// is it last job?
                auto reqB = std::make_shared<BroadcastBatchRequest<IdB>>(batchJobCount == 1);// is it last job?
                for(int32_t kt = 0; kt < KT; ++kt) {
                    for(auto rowIdx: rowIndicesSet) {
                        reqA->addIndex(rowIdx, kt);
                        reqA->broadCastList.insert(matrixA->owner(rowIdx, kt));
                    }

                    for(auto colIdx: colIndicesSet) {
                        reqB->addIndex(kt, colIdx);
                        reqB->broadCastList.insert(matrixB->owner(kt, colIdx));
                    }
                }
                this->addResult(reqA);
                this->addResult(reqB);
