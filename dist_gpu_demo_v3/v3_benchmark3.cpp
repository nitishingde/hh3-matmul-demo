...
int main(int argc, char *argv[]) {
    ...
    MpiGlobalLockGuard mpiGlobalLockGuard(&argc, &argv, p, q, MPI_THREAD_MULTIPLE);
    ...
    auto strategy = MMD_WindowStrategy3<MatrixType, IdA, IdB, IdC>();
    //warmup
    auto time = strategy.builder(gp, gq, wh, ww, d, l, productThreads, T).executeImpl(matrixA, matrixB, matrixC, deviceIds, mpiComm, path + "window" + "_" + std::to_string(getNodeId()) + ".dot");
    if(isRootNodeId()) {
