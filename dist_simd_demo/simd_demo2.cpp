    ...
    for(int64_t row = 0; row < matrix->matrixNumRowTiles(); ++row) {
        for(int64_t col = 0; col < matrix->matrixNumColTiles(); ++col) {
            ...
            if(auto tile = matrix->tile(row, col); !isRootNodeId() and tile != nullptr) {
                ...
                checkMpiErrors(MPI_Send(tile->data(), tile->byteSize(), MPI_CHAR, 0, tagId, matrix->mpiComm()));
            }
            else if(isRootNodeId() and tile == nullptr) {
                MPI_Status mpiStatus;
                const auto tagId = row*matrix->matrixNumColTiles() + col;
                checkMpiErrors(MPI_Recv(static_cast<void*>(tempTile), tw*th*sizeof(MatrixType), MPI_CHAR, matrix->owner(row, col), tagId, matrix->mpiComm(), &mpiStatus));
...
int main(int argc, char *argv[]) {
    ...
    const auto matrixA = std::make_shared<TwoDBlockCyclicMatrix<MatrixType, IdA>>(memoryType, M, K, T, p, q, gridComm);
    const auto matrixB = std::make_shared<TwoDBlockCyclicMatrix<MatrixType, IdB>>(memoryType, K, N, T, p, q, gridComm);
    const auto matrixC = std::make_shared<TwoDBlockCyclicMatrix<MatrixType, IdC>>(memoryType, M, N, T, p, q, gridComm);
    auto             C = getMatrixToRoot<MatrixType, IdC>(matrixC);

    auto mmd  = MMD_Simd2<MatrixType, IdA, IdB, IdC>(groupCommA, groupCommB);
    if(isRootNodeId()) {
    ...
    const auto time = mmd.builder(productThreads, l).executeImpl(matrixA, matrixB, matrixC, gridComm, path + "tp" + std::to_string(getNodeId()) + ".dot");
